...
function showLesson(lessonId) {
  ...
  view.innerHTML = `
    ...
    <div class="lesson-page" id="lessonPrintArea">
      ...
      ${(lesson.bookPrompt || lesson.bookPages) ? `
      <div class="lesson-book-note">
        ${lesson.bookPrompt ? `<p><strong>من صفحة الدرس:</strong>${lesson.bookPrompt}</p>` : ''}
        ${lesson.bookPages ? `<span class="lesson-book-ref">مرجع الكتاب: ${lesson.bookPages}</span>` : ''}
      </div>` : ''}

      <div class="lesson-section">
        <h3><span class="section-icon">أهداف</span> أهداف الدرس</h3>
      ...
      ${conceptsHtml ? `
      <div class="lesson-section">
        <h3><span class="section-icon">مفاهيم</span> المفاهيم الأساسية</h3>
        ${conceptsHtml}
      </div>` : ''}

      ${lesson.content ? `
      <div class="lesson-section">
        <h3><span class="section-icon">نص</span> محتوى الدرس</h3>
...
function renderGamesSection() {
  ...
  view.innerHTML = `
    <div class="section-header">
      ...
      <p>اختر درساً للعب النشاط التفاعلي المرتبط به وجمع النقاط.</p>
    </div>
